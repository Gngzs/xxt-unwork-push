...
print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())+"载入成功，正在定时运行")
...
def job_unwork():
    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())+push_unwork())


def job_unwork24():
    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())+push_unwork24())
